---
name: nature-reader
description: Build full-paper, figure/table-aware, source-grounded Markdown readers for journal or conference papers from PDF, DOI, arXiv, publisher HTML, or pasted text. Use whenever the user asks to read, analyze, or process a paper, extract figures or tables into the right positions, preserve figure/table placement near relevant prose, or keep exact source anchors for every block. This skill must not degrade into a summary-only output unless the user explicitly asks for a summary. Also trigger on general paper-reading and literature analysis requests.
version: 2.0.0
...
1. Core principles (`core/principles.md`) — structured reader by default, analyze for meaning, never degrade to a summary, copyright caution.
...
Build the Terminology Ledger as you process (`../_shared/core/terminology-ledger.md`); it becomes the `paper.md` recurring-term table and the `source_map.json` glossary.

If constraints prevent full processing, still create a draft reader and label missing pages, figures, or low-confidence crops in `processing_notes.md`. Do not switch to summary mode.
