...
## 2. Build a full-document source map before processing
...
For each block, capture: page number, block type, text content, reading-order index, nearby figure or table references, first substantive figure/table mention when applicable, and confidence level when extraction is uncertain.
...
## 3. Process text conservatively

Process every extractable substantive block with these rules:

- preserve technical terms and standard domain expressions
...
If a sentence contains multiple claims, keep the text readable but do not split away the original evidence chain. Build the Terminology Ledger (`../../../_shared/core/terminology-ledger.md`) as you process so recurring terms stay consistent across the whole document.

## 4. Extract and place figures and tables near the relevant discussion

Crop each figure/table into `assets/` and place it near its first substantive mention, keeping the caption attached with clear caption text. For the full placement and tight-crop rules, and the figure/table block shape, open `references/figure-extraction.md`.
...
- paragraph-level structured blocks for all extractable substantive text
...
- a short `critical reading notes` section only after the body text, not as a replacement for it
