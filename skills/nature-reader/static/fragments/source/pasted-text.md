...
- Treat the pasted text as the source of truth. Build the source map and structured blocks from it.
- Page numbers may be unknown. Use sequential block IDs (`S001`, `S002`, ...) and, where the paste shows section headings, use section-level anchors instead of page anchors. Note in `processing_notes.md` that page anchors are unavailable.
...
- Keep the structured reader format; do not collapse pasted prose into a summary just because layout metadata is missing.
