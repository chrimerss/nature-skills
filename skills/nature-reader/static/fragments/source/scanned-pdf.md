...
- Record a confidence level for each block in the source map, and mark low-confidence blocks explicitly in `processing_notes.md` rather than guessing.
...
- If pages are skewed, rotated, or partly cut off, note the affected pages and extract only what is legible.
