#!/usr/bin/env python3
"""Render a structured patent draft JSON file as DOCX."""
...
def add_heading(document: Document, text: str, level: int = 1) -> None:
    ...
    set_run_font(run, "Arial", 16 if level == 1 else 14, bold=True)
...
def add_specification(document: Document, data: dict, figure_dir: Path | None = None) -> None:
    add_heading(document, "Specification")
    ...
    sections.extend(
        [
            ("Summary of the Invention", invention.get("problem", []) + invention.get("solution", [])),
            ("Beneficial Effects", invention.get("beneficial_effects", [])),
            ("Brief Description of the Drawings", spec.get("figure_descriptions", [])),
        ]
    )
    ...
    if equations:
        add_heading(document, "Equations and Symbol Definitions", level=2)
    ...
    if figure_dir:
        ...
        if figures:
            add_heading(document, "Drawings", level=2)
        for figure in figures:
            ...
            caption_run = caption.add_run(f"Fig. {figure['number']} {figure.get('title', '')}")
            set_run_font(caption_run, "Arial", 11)

    add_heading(document, "Detailed Description of the Embodiments", level=2)
...
def add_abstract_figure(
    ...
    add_heading(document, "Abstract Drawing")
    figure = abstract_figure(data)
    if figure:
...
def add_review_appendix(document: Document, data: dict) -> None:
    ...
    if ledger:
        add_heading(document, "Evidence Ledger", level=2)
        table = document.add_table(rows=1, cols=6)
        table.style = "Table Grid"
        headers = ("ID", "Technical Feature", "Source Location", "Technical Role", "Effect", "Support Status")
    ...
    for heading, key in (
        ("Support Findings", "support_findings"),
        ("Consistency Findings", "consistency_findings"),
        ("Inventor Questions", None),
    ):
        add_heading(document, heading, level=2)
